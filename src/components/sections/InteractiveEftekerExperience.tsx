...
export const InteractiveEftekerExperience: FC = () => {
  ...
  return (
    <section id="efteker-experience" className="py-24 bg-gradient-to-b from-[#ffffff] via-[#eff6ff]/30 to-[#f4f4f7] border-t border-black/[0.04] overflow-hidden font-cairo">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Section Header */}
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-blue-100/80 border border-blue-300/60 text-blue-800 text-xs font-bold shadow-xs">
          ...
          <h2 className="text-3xl sm:text-5xl lg:text-6xl font-black text-[#1d1d1f] tracking-tight leading-tight">
            {t.eftekerExpTitle} <span className="text-[#2563eb]">{t.eftekerExpTitleAccent}</span>
          </h2>
        ...
        <div className="apple-card p-6 sm:p-12 bg-white/90 border border-blue-100 shadow-2xl relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
            ...
            <div className="lg:col-span-6 space-y-6 text-start order-2 lg:order-1">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {availableBubbles.map((bubble, idx) => (
                      <button
                        ...
                        className="px-3.5 py-2 rounded-xl bg-amber-50 hover:bg-amber-100 border border-amber-200/80 text-xs font-bold text-amber-900 shadow-xs hover:scale-105 active:scale-95 transition-all flex items-center gap-1.5 cursor-pointer"
                      >
                        <span>＋ {bubble}</span>
                      </button>
                    ))}
              ...
              <div className="p-5 rounded-2xl bg-neutral-50 border border-black/[0.06] space-y-3">
                <div className="flex items-center justify-between text-xs font-bold">
                  ...
                  <span className="text-blue-600 font-mono text-sm">{clarityPercent}%</span>
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                  {capturedThoughts.map((c, i) => (
                    <div
                      key={i}
                      className="p-3 rounded-xl bg-white border border-blue-100 shadow-xs flex items-center justify-between text-xs"
                    >
                      <span className="font-semibold text-[#1d1d1f]">{c}</span>
                      <span className="text-[10px] text-blue-600 font-bold px-2 py-0.5 rounded-full bg-blue-50 shrink-0">
            ...
            <div className="lg:col-span-6 flex flex-col items-center justify-center text-center order-1 lg:order-2">
              <div className="relative group">
                ...
                <div className="relative w-64 h-64 sm:w-80 sm:h-80 rounded-3xl p-4 bg-white/80 backdrop-blur-md border border-blue-100 shadow-2xl flex items-center justify-center group-hover:scale-105 transition-transform duration-500">
                  <img
                ...
                <div className="absolute -bottom-3 -left-3 px-3.5 py-1.5 rounded-full bg-white text-[#1d1d1f] border border-black/[0.08] text-xs font-bold shadow-lg flex items-center gap-1.5">
                  <Heart className="w-3.5 h-3.5 text-red-500" />
                  <span>{language === 'ar' ? 'راحة بال تامة' : 'Total Peace of Mind'}</span>
                </div>
              </div>

              <div className="mt-8 text-xs text-[#6e6e73] font-medium">
                {language === 'ar'
